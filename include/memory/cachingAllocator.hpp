...
struct Block {
    ...
    Block* next;
    cudaStream_t stream;
    int device;  // Track which GPU device this block belongs to

    Block(void* a = nullptr, size_t s = 0, PoolType p = PoolType::SMALL, cudaStream_t str = 0, int dev = 0)
        : addr(a), size(s), active(false), pool_type(p),
          prev(nullptr), next(nullptr), stream(str), device(dev) {}
};
...
private:
    ...
    Block* mergeAdjacent(Block* block, StreamInternal& internal);
};
